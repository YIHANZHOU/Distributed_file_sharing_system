...
public class CoordinatorHandler implements ServerThrift.Iface {
  // define shared variables and locks for synchronization
  private int RN;  // number of read file
  private int WN;
  private ArrayList<Address> ServerList;  // available compute node list
  ...
  public String (String inputDir) {
    ...
    for (int i = 0; i < listOfFiles.length; i++) {
      if (listOfFiles[i].isFile()) {
        numOfFiles++;public String (String inputDir) {
    // time point
    long starttime = System.currentTimeMillis();
    System.out.println("================ Received a new Job! ================");
    if(policy == 1) {
